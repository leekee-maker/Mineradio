...
## SelfRadio Design Decision

- Selected direction: option 3, the bright soft-light visual language that matches the rounded SelfRadio gradient logo.
- Keep the mobile experience friendly, spacious, lyric-led, and visibly cross-platform; avoid the desktop app's heavy black presentation.
- Core prototype interactions are playback, synchronized lyrics, queue, source switching, and sound-scene selection.
